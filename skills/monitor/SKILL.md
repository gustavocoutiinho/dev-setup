...
- **Custo:** OpenRouter tem **teto de $5 por chave** (4 chaves, `gustavo@minerbz.com.br`, $10 de crédito); avise a que se aproxima antes de o produto cair. Vercel: cruze com [[faxina-web]] pra achar deploy morto/duplicado. Supabase: crons de alta frequência (`*/15`) multiplicam invocação. **Ex-cliente sangrando:** ✅ resolvido 24/07 — `miner-weekly-report-mgtc` e os outros 4 crons `weekly-report-template` (`-nmtl`,`-nmpr`,`-stkr-fr`,`-stkr-mm`) foram pausados; achado maior: essa edge só lê e devolve na resposta HTTP, o cron via `net.http_post` fire-and-forget nunca entregava o resultado em lugar nenhum (ver [[robo]]). Windsor ~R$137k/mês é **verba do cliente**, vigie a variação, não o valor absoluto.
...
- **Radar de bloqueios:** lista viva em `~/Documents/Claude/bloqueios-ativos.md` (criada 24/07, ~28 itens `[VC]` extraídos de `oportunidades-portais-relatorios.md` + status real). Ordene por prazo quente: **#112 VPS ForYou já venceu 09/06**, **#108 token Meta vence 22/08**. Ao Gustavo resolver um item, **destrave só depois de confirmar que a credencial funciona**, marque ✅ no arquivo, e aponte a skill que volta a rodar ([[robo]]). Antes de cobrar item como pendente, confirme com grep/curl — o plano de 18/07 errou 3 vezes (achou "morto" projeto que estava vivo).
