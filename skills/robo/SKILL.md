...
- **Relatório semanal:** ⚠️ **achado 24/07, corrige o que este SKILL dizia antes:** a edge `weekly-report-template` só **lê** `miner_reports_log` (`select` por cliente/período) e devolve `{html,markdown}` na resposta HTTP — ela **não grava** nada. Os 5 crons `miner-weekly-report-<slug>` (`-mgtc`, `-nmtl`, `-nmpr`, `-stkr-fr`, `-stkr-mm`, todos `0 12 * * 1`) chamavam ela via `util.invoke_edge`, que usa `net.http_post` **fire-and-forget**: dispara e não guarda a resposta em lugar nenhum. Não existe function de email/notify/deliver no projeto que busque essa resposta depois. Ou seja: **nenhum desses 5 crons jamais entregou relatório em lugar nenhum**, mesmo ativos — geravam e o resultado ia pro vácuo. **Todos os 5 pausados em 24/07** (`cron.alter_job(active:=false)`, reversível). Se quiser o relatório formato Stalker de verdade, chame `weekly-report-template` sob demanda (não por cron) e mande o `html`/`markdown` pra algum destino real (e-mail, Slack, portal). Fonte dos números = ingestors (`meta-ingestor`/`windsor-ingestor`/`instagram-ingestor`, diários 9h-9h15 BRT).
